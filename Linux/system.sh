...
free -m -h | awk -F " " 'NR==2{print "Memory Usage:""\nFree:" $3,"\nUsed:" $4}' > ~/backups/freemem/free_mem.txt

# Disk usage output to a disk_usage.txt file
df -ah --total | awk -F" " '{print "Used: "$1,$3}' > ~/backups/diskuse/disk_usage.txt

# List open files to a open_list.txt file
lsof > ~/backups/openlist/open_list.txt

# Free disk space to a free_disk.txt file
df -ah --total | awk -F" " '{print "Free: "$1,$4}' > ~/backups/freedisk/free_disk.txt
